...
# tmfg
def generate_cov_tmfg(return_matrix):

    df = return_matrix
    in_sample_mean = df.mean()*252
    cov = df.cov()*252
    W = correlation_from_covariance(cov)**2 
    
    model = TMFG()
    cliques, seps, adj_matrix = model.fit_transform(W,cov, output='logo')
    adj_matrix = pd.DataFrame(adj_matrix, columns=['col_{}'.format(i) for i in range(len(adj_matrix[0]))])   
    return in_sample_mean, cov, adj_matrix
